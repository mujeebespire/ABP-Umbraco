window.addEventListener("DOMContentLoaded", (event) => {
  ...
  slider.oninput = function () {
    ...
    gsap.to(greenCounter, 0.8, {
      ...
      onUpdate: function () {
        greenCount.innerText = `£${greenCounter.val.toFixed(1)}m`;
      },
      ease: Power4.easeOut,
    });
    if (value === 0) {
    ...
    if (isGreen) {
      for (const [key, objValue] of Object.entries(yearArr[0].green)) {
        if (key !== "total") {
          const fieldName = "green-" + key + "-txt";
          const textFieled = document.getElementById(fieldName);
          textFieled.innerText = "£" + objValue + "m";
        }
      }
      gsap.to(".green-devision", 0.4, {
        scale: (index, target) => {
  ...
  fetch("data-new.json")
    ...
    .then((json) => {
      ...
      greenCount.innerText = `£${years[0].green.total}m`;
